...
## Iteration 132 - README banner pinned to top
- Pinned the Zed Zed Zed ASCII banner to the top of the README so it always appears before all changelog and project text.
...
## Iteration 120 - Zombie movement path fix
- Reworked the mod's internal EdgeWander path so normal wandering zombies move around the colony approaches and interior lanes instead of selecting cells that drag them toward the map edge.
- This should stop debug-spawned and naturally wandering zombies from immediately walking off the map when they are supposed to wander or push the base.


## Iteration 121 - Zombie status cleanup
- Kept zombie infection as its own visible status and kept Reanimating as its own visible fake death recovery status.
- Hid redundant helper statuses like fully reanimated, variant strain markers, drowned environment markers, runt age markers, limb decay, and rotting wound so the health tab is less cluttered while the underlying systems still work.
- Updated infection display helpers so infection UI prefers the actual zombie infection status instead of surfacing the hidden reanimated helper state.
