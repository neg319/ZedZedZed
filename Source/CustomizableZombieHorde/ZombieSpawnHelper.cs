...
namespace CustomizableZombieHorde
{
    ...
    public static class ZombieSpawnHelper
    {
        ...
        private static void TrySpawnRareRuntWithBiters(Map map, Faction faction, int biterSpawnCount, IntVec3 anchorCell, ZombieSpawnEventType behavior, List<Pawn> spawnedPawns = null)
        {
            ...
            if (map == null || faction == null || biterSpawnCount < 3 || !anchorCell.IsValid || crawlerKind == null)
            {
                return;
            }
            ...
            try
            {
                ...
                if (SpawnZombiePawn(map, crawler, spawnCell, behavior))
                {
                    spawnedPawns?.Add(crawler);
                }
                else
                {
                    crawler.Destroy(DestroyMode.Vanish);
                }
            }
            catch (System.Exception ex)
            {
                Log.Error($"[ZedZedZed] Failed to spawn rare crawler with biter group: {ex}");
            }
        }
