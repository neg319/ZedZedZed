...
namespace CustomizableZombieHorde
{
    public static class ZombieDefUtility
    {
        ...
        public static void ApplyDynamicLabels()
        {
            ...
            foreach (string crawlerKindDefName in new[] { "CZH_CrawlerKind", "CZH_GraveRuntKind", "CZH_RuntKind" })
            {
                PawnKindDef crawlerKind = DefDatabase<PawnKindDef>.GetNamedSilentFail(crawlerKindDefName);
                if (crawlerKind != null)
                {
                    crawlerKind.label = "crawler";
                }
            }

            foreach (string crawlerThingDefName in new[] { "CZH_Crawler", "CZH_GraveRunt", "CZH_Runt" })
            {
                ThingDef crawlerThing = DefDatabase<ThingDef>.GetNamedSilentFail(crawlerThingDefName);
                if (crawlerThing != null)
                {
                    crawlerThing.label = "crawler";
                    crawlerThing.description = "A tiny, frantic undead crawler left behind by the most grotesque boomer bursts. It moves fast, bites lightly, and scurries straight for the living.";
                }
            }
