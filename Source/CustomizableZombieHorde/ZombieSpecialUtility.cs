...
namespace CustomizableZombieHorde
{
    public static class ZombieSpecialUtility
    {
        ...
        private static void TrySpawnBoomerRunt(Pawn sourcePawn, Map map, IntVec3 pos)
        {
            ...
            PawnKindDef crawlerKind = ZombieDefOf.CZH_CrawlerKind ?? ZombieDefOf.CZH_GraveRuntKind ?? ZombieDefOf.CZH_RuntKind;
            if (sourcePawn == null || map == null || !pos.IsValid || !pos.InBounds(map) || crawlerKind == null)
            {
                return;
            }
            ...
            try
            {
                ...
                Pawn crawler = PawnGenerator.GeneratePawn(crawlerKind, faction);
                if (crawler == null)
                {
                    return;
                }
                ...
                crawler.mindState?.mentalStateHandler?.TryStartMentalState(MentalStateDefOf.ManhunterPermanent, forceWake: true);
            }
            catch (Exception ex)
            {
                Log.Error($"[ZedZedZed] Failed to spawn crawler from female boomer burst: {ex}");
            }
        }
