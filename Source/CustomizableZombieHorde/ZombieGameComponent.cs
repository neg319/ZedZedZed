...
namespace CustomizableZombieHorde
{
    public sealed class ZombieGameComponent : GameComponent
    {
        ...
        public bool DebugSpawnCrawler()
        {
            Map map = GetDebugTargetMap();
            PawnKindDef kind = ZombieDefOf.CZH_CrawlerKind ?? ZombieDefOf.CZH_GraveRuntKind ?? ZombieDefOf.CZH_RuntKind;
            if (map == null || kind == null)
            {
                return false;
            }
            ...
            Pawn crawler = PawnGenerator.GeneratePawn(kind, faction);
            if (crawler == null)
            {
                return false;
            }

            GenSpawn.Spawn(crawler, spawnCell, map);
            RegisterBehavior(crawler, ZombieSpawnEventType.AssaultBase);
            ZombieUtility.PrepareSpawnedZombie(crawler);
            ZombieUtility.AssignInitialShambleJob(crawler, ZombieSpawnEventType.AssaultBase);
            ZombieUtility.EnsureZombieAggression(crawler);
            Messages.Message("A debug crawler has been spawned.", crawler, MessageTypeDefOf.NeutralEvent);
            RefreshCurrentMapCount();
            return true;
        }
